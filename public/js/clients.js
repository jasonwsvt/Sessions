class Clients {
    ...
    get current()             { console.log(this._current, this._clients); return this.findById(this._current); }
    set current(id)           { this._current = id; }
    findById(id)              { return this._clients.find(client => (client._id == id)); }
    findByName(name)          { return this._clients.find(client => (client.name == name)); }
